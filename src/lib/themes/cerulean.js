...
export default {
  ...
  modal: `${inputRounded} z-[2] w-auto h-auto top-auto bottom-[max(2rem,_env(safe-area-inset-bottom))] left-1/2 transform -translate-x-1/2 bg-gradient-to-b from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 w-full max-w-[calc(100vw_-_1rem)] p-[1.5rem] pr-[1.5rem] pl-[1.5rem] pb-[1.5rem]`,
  nav: 'space-y-[2rem] z-[2] transition duration-200 transform fixed top-0 right-0 w-[calc(100%_-_4rem)] h-screen max-h-screen overflow-y-auto items-stretch pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] bg-gradient-to-b from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 lg:right-auto lg:left-0 lg:top-0 lg:w-auto lg:pl-[3rem]',
