    ...
    user === undefined || $collections?.roles === undefined || $collections?.routes === undefined
      ...
      : user.roles
          .reduce((arr, _roleId) => {
            ...
            // loop through collection routes
            role.collections.map((_collectionId) => {
              // find route
              const route = $collections['quick-collections'].find(
                (currentRoute) => currentRoute._id.toString() === _collectionId.toString()
              );
              ...
              if (
                !arr[groupIndex].items.find((item) => item._id.toString() === route._id.toString())
              )
                arr[groupIndex].items.push({
                  _id: route._id,
                  href: `/collections/${route.href}`,
