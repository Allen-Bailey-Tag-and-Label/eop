<script>
  ...
  const statusChangeHandler = async (e, pcr, i) => {
    modal.spinner.show();
    await serverFetch({method:'PATCH', href:`/api/datatable/pay-change-requests?_id=${pcr._id}`, body : {status: e.target.value}})
    pcrs[i].status = e.target.value
    modal.spinner.hide();
  }
          ...
          <tr>
            ...
            <td class="text-center">
              {#if !admin}
                {pcr.status}
              {:else}
                <Select options={statusOptions} value={pcr.status} on:change={e => statusChangeHandler(e, pcr, i)} />
              {/if}
            </td>
            <td class="text-center">
              <div class="flex space-x-[.5rem]">
                <Buttons.Icon on:click={()=>downloadClickHandler(pcr)}><Icon src={Download} class="w-[24px] h-[24px]"/></Buttons.Icon>
