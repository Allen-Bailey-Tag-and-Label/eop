<script>
  ...
  const getJobTitles = async () => {
    const data = await serverFetch(
      `/api/datatable/job-titles?sort=${JSON.stringify({ jobTitle: 1 })}`
    );
    selects.jobTitleId = [
      { label: "", value: "" },
      ...data.rows.map((row) => {
  ...
  const getRoles = async () => {
    const data = await serverFetch(
      `/api/datatable/roles?sort=${JSON.stringify({ name: 1 })}`
    );
  ...
  const submitHandler = async () => {
    ...
    const user = Object.keys(body).reduce((obj, key) => {
      ...
      if (body[key]?.type === "date")
        obj[key] = moment(obj[key], "MM.DD.YYYY").format("x");
      return obj;
    }, {});
    const data = await serverFetch({
      method: "POST",
    ...
    const modalInnerHTML = `Successfully added user "${body.firstName.value} ${
      body.lastName.value
    }".  Copy text below to have user verify account.<br><br>Please verify your account with the link below with password <span class="font-bold">${password}</span>:<br><a href="${
      import.meta.env.VITE_BASE_URL
    }/users/verify?accessToken=${accessToken}">Verify</a>`;
    modal.spinner.hide();
  ...
  onMount(async () => {
    await Promise.all([getJobTitles(), getRoles()]);
    loaded = true;
  });
</script>
...
<Section>
  ...
  {#if loaded}
    <form
      ...
      class="flex flex-col mt-[24px] space-y-[16px] max-w-[500px] w-full mx-auto">
